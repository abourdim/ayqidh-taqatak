/* ═══════════════════════════════════════════════════════════
   NARRATOR.JS — Audiobook narrator for Ayqidh Taqatak
...
(function() {
  ...
  const STATE = {
    ...
    cardIndex: 0,
    cards: [],
    tabOrder: ['about','causes','energies','solutions'],
    tabIndex: 0,
  ...
  function extractCards(panel) {
    ...
    if (tabName !== 'about') {
      ...
      if (title) {
        cards.push({
          el: title.closest('.tab-content') || title,
    ...
    if (tabName === 'about') {
      ...
      panel.querySelectorAll('.expand-card').forEach(el => {
        const text = cleanText(el.textContent);
        if (text) cards.push({ el, text, type: 'cause' });
      });
    } else if (tabName === 'energies') {
      panel.querySelectorAll('.expand-card').forEach(el => {
        const text = cleanText(el.textContent);
        if (text) cards.push({ el, text, type: 'energy' });
      });
      ...
      const daily = panel.querySelector('.daily-tip-card');
      if (daily) cards.push({ el: daily, text: cleanText(daily.textContent), type: 'daily' });
      const challenge = panel.querySelector('.challenge-card');
      if (challenge) cards.push({ el: challenge, text: cleanText(challenge.textContent), type: 'challenge' });
    }
    return cards;
  }
  ...
  function cleanText(text) {
    ...
    if (l === 'fr') {
      clean = clean
        ...
        .replace(/Ayqidh Taqatak/gi, 'Aykiz Takatak')
    ...
    if (l === 'en') {
      clean = clean
        ...
        .replace(/Ayqidh Taqatak/gi, 'Ay-kiz Ta-ka-tak')
  ...
  function readCurrentCard() {
    ...
    // Open expand cards so content is visible
    if ((card.type === 'cause' || card.type === 'energy' || card.type === 'solution') && card.el && !card.el.classList.contains('open')) {
  ...
  function setupMediaSession() {
    ...
    navigator.mediaSession.metadata = new MediaMetadata({
      title: l === 'ar' ? 'أيقظ طاقتك' : l === 'fr' ? 'Éveille ton Énergie' : 'Awaken Your Energy',
  ...
  function onSpeedChange(val) {
    ...
    localStorage.setItem('ayqidh-narrator-speed', STATE.speed);
  }
  ...
  function onPitchChange(val) {
    ...
    localStorage.setItem('ayqidh-narrator-pitch', STATE.pitch);
  }
  ...
  function onKaraokeToggle(checked) {
    ...
    localStorage.setItem('ayqidh-narrator-karaoke', checked);
  }

  function onAutoScrollToggle(checked) {
    STATE.autoScroll = checked;
    localStorage.setItem('ayqidh-narrator-autoscroll', checked);
  }

  function onDuoToggle(checked) {
    STATE.duoReading = checked;
    localStorage.setItem('ayqidh-narrator-duo', checked);
  }
  ...
  function loadSettings() {
    STATE.speed = parseFloat(localStorage.getItem('ayqidh-narrator-speed')) || 1;
    STATE.pitch = parseFloat(localStorage.getItem('ayqidh-narrator-pitch')) || 1;
    STATE.karaokeEnabled = localStorage.getItem('ayqidh-narrator-karaoke') !== 'false';
    STATE.autoScroll = localStorage.getItem('ayqidh-narrator-autoscroll') !== 'false';
    STATE.duoReading = localStorage.getItem('ayqidh-narrator-duo') === 'true';
  }
  ...
  function initNarrator() {
    ...
    const origLangFn = window.applyLang || window.setLang;
    const hookName = window.applyLang ? 'applyLang' : 'setLang';
    if (origLangFn) {
  ...
  function injectSpeakButtons() {
    ...
    var selectors = [
      '.about-card', '.expand-card', '.daily-tip-card', '.challenge-card'
    ];
  ...
  var origRenderCauses = window.renderCauses;
  if (origRenderCauses) {
    window.renderCauses = function() { origRenderCauses(); setTimeout(injectSpeakButtons, 100); };
  }
  var origRenderEnergies = window.renderEnergies;
  if (origRenderEnergies) {
    window.renderEnergies = function() { origRenderEnergies(); setTimeout(injectSpeakButtons, 100); };
  }
  var origRenderSolutions = window.renderSolutions;
  if (origRenderSolutions) {
    window.renderSolutions = function() { origRenderSolutions(); setTimeout(injectSpeakButtons, 100); };
  }
  var origRenderScenarios = window.renderScenarios;
  if (origRenderScenarios) {
    window.renderScenarios = function() { origRenderScenarios(); setTimeout(injectSpeakButtons, 100); };
  }
  var origRenderDailyTip = window.renderDailyTip;
  if (origRenderDailyTip) {
    window.renderDailyTip = function() { origRenderDailyTip(); setTimeout(injectSpeakButtons, 100); };
  }
  var origRenderChallenge = window.renderChallenge;
  if (origRenderChallenge) {
    window.renderChallenge = function() { origRenderChallenge(); setTimeout(injectSpeakButtons, 100); };
  }
